...
struct GreenNodeFlags(u8);

impl GreenNodeFlags {
    const HAS_COMMENTS: Self = Self(1 << 0);
    const HAS_SKIPPED: Self = Self(1 << 1);
...
#[cfg(target_pointer_width = "64")]
static_assert!(mem::size_of::<GreenNodeFlags>() == 1);
#[cfg(target_pointer_width = "64")]
...
impl GreenNodeData {
    ...
    pub(crate) fn has_comments(&self) -> bool {
        self.flags().contains(GreenNodeFlags::HAS_COMMENTS)
    }

    #[inline]
    pub(crate) fn has_skipped(&self) -> bool {
        self.flags().contains(GreenNodeFlags::HAS_SKIPPED)
    }

    #[inline]
    fn flags(&self) -> GreenNodeFlags {
        self.header().flags
    }
...
impl GreenNode {
        ...
        I::IntoIter: ExactSizeIterator,
    {
        ...
        let mut flags = GreenNodeFlags::default();
                ...
                Some(el) => {
                    ...
                    match &el {
                        ...
                        NodeOrToken::Token(token) => {
                            ...
                            {
                                if piece.kind().is_comment() {
                                    flags.insert(GreenNodeFlags::HAS_COMMENTS);
                                }
                                if piece.kind().is_skipped() {
                                    flags.insert(GreenNodeFlags::HAS_SKIPPED);
                                }
                            }
                        }
                    }
